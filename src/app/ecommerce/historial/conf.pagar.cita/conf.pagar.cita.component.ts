...
export class ConfPagarCitaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // generar transacción en BD
    // const arrParams = {
    //   purchaseOperationNumber: this.data.purchaseOperationNumber,
    //   purchaseVerification: this.data.purchaseVerification,
    // };
    const arrParams = {
      idcita: this.data.idcita,
    }
    this.ngxLoader.start();
    this.citaService.registrarTransaccion(arrParams).subscribe(
      r => {
